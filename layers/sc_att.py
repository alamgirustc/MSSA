...
class SCAtt(BasicAtt):
    def __init__(self, mid_dims, mid_dropout):
        ...
        self.attention_last = nn.Linear(mid_dims[-2], 1)  # Spatial attention layer

    def forward(self, att_map, att_mask, value1, value2, geo_feats=None):
        # Apply basic attention if present
        ...
        # Compute spatial attention (alpha_spatial)
        alpha_spatial = self.attention_last(att_map)
        alpha_spatial = alpha_spatial.squeeze(-1)
        
        # Apply the attention mask (if present) to spatial attention
        if att_mask is not None:
            alpha_spatial = alpha_spatial.masked_fill(att_mask == 0, -1e9)
        
        # Normalize spatial attention using softmax
        alpha_spatial = F.softmax(alpha_spatial, dim=-1)

        # Perform attention on value2 with spatial attention
        if len(alpha_spatial.shape) == 4:  # for xtransformer (batch_size * head_num * seq_num * seq_num)
            value2 = torch.matmul(alpha_spatial, value2)
        else:  # standard sequence attention (batch_size * seq_num * feature_dim)
            value2 = torch.matmul(alpha_spatial.unsqueeze(-2), value2).squeeze(-2)

        # Final attention output based on value1 and value2
        attn = value1 * value2
        return attn
