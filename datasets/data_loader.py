...
def sample_collate(batch):
    indices, input_seq, target_seq, gv_feat, att_feats, geo_feats = zip(*batch)
    ...
    geo_feat_arr = []  # Array for geometric features
    ...
    for i, num in enumerate(atts_num):
        ...
        # Add geometric features to the array with padding or truncation
    ...
    return indices, input_seq, target_seq, gv_feat, att_feats, att_mask, geo_feats


def sample_collate_val(batch):
    indices, gv_feat, att_feats, geo_feats = zip(*batch)
    ...
    return indices, gv_feat, att_feats, att_mask, geo_feats
...
def load_val(image_ids_path, gv_feat_path, att_feats_folder, geo_feats_folder):
