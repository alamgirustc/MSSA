...
class Trainer(object):
    ...
    def snapshot_path(self, name, epoch):
        """
        Generate the snapshot path dynamically based on the PBS job environment.
        If RL mode is enabled, the snapshot is saved in the RL-specific directory.
        """
        # Hardcoded project name
        project_name = 'LSTM_s1290L20g'

        # If RL mode is enabled (based on --resume flag), use the appropriate folder
        if hasattr(self.args, 'resume') and self.args.resume > 0:
            snapshot_folder = f'/gdata/Alamgir/mscoco/model/{project_name}/experiments/xlan_rl/snapshot'
        else:
            snapshot_folder = f'/gdata/Alamgir/mscoco/model/{project_name}/experiments/xlan/snapshot'

        # Ensure the snapshot folder exists
        os.makedirs(snapshot_folder, exist_ok=True)

        # Return the full path to save the checkpoint
        return os.path.join(snapshot_folder, f"{name}_{epoch}.pth")
        
    def save_model(self, epoch):
        """
        Save the model checkpoint dynamically based on the PBS working directory.
        Ensure the snapshot folder exists and create it if not.
        """
        ...
        # Dynamically create the snapshot folder if it does not exist
        os.makedirs(snapshot_folder, exist_ok=True)

        # Save the model's state_dict to the determined snapshot path
    ...
    def make_kwargs(self, indices, input_seq, target_seq, gv_feat, att_feats, att_mask, geo_feats):
        ...
        kwargs = {
            ...
            cfg.PARAM.GEO_FEATS: geo_feats  # Add geo_feats to kwargs
        }
        return kwargs
    ...
    def forward(self, kwargs):
        if self.rl_stage == False:
            #print(f"main geo_feats in forward: {kwargs.get(cfg.PARAM.GEO_FEATS)}")  # Debug statement
            ...
            geo_feats = kwargs[cfg.PARAM.GEO_FEATS]  # Add this line to extract geo_feats
            #print(f"main geo_feats in RL forward: {geo_feats}")  # Debug statement
            ...
            geo_feats = utils.expand_tensor(geo_feats, cfg.DATA_LOADER.SEQ_PER_IMG)  # Add this line to expand geo_feats
    ...
    def train(self):
        ...
        for epoch in range(cfg.SOLVER.MAX_EPOCH):
            ...
            for _, (indices, input_seq, target_seq, gv_feat, att_feats, att_mask, geo_feats) in enumerate(
                ...
                geo_feats = geo_feats.cuda()  # Add this line to transfer geo_feats to GPU
